...
import { canCrossMazeGate, canEnterMazePosition, FINISH_PORTAL, MAP_SIZE, mazeDoors, mazeRooms, mazeWalls } from '../lib/backrooms';
...
export function BackroomsMap({ onCaught, onFinish, room }: BackroomsMapProps) {
  ...
  const [owner, setOwner] = useState<Point>({ x: 100, y: 154 });
  ...
  useEffect(() => {
    function openHack(event: KeyboardEvent) {
      const distance = Math.hypot(player.x - FINISH_PORTAL.x, player.y - FINISH_PORTAL.y);
      if (event.code === 'KeyE' && distance < 13 && !portalOpen) setHackOpen(true);
    }
  ...
  useEffect(() => {
    ...
    function tick(now: number) {
      ...
      staminaRef.current = running ? Math.max(0, staminaRef.current - .25 * delta) : Math.min(100, staminaRef.current + .13 * delta);
      ...
      const smoothing = hasInput ? .09 : .065;
      ...
      velocity.current.x += (moveX - velocity.current.x) * smoothing * delta;
      velocity.current.y += (moveY - velocity.current.y) * smoothing * delta;
      setPlayer((current) => {
        ...
        const canMoveX = canEnterMazePosition(px, current.y, unlockedRef.current)
          && canCrossMazeGate(current.x, px, unlockedRef.current);
        const x = canMoveX ? px : current.x;
        const y = canEnterMazePosition(x, py, unlockedRef.current) ? py : current.y;
      ...
      setOwner((current) => {
        ...
        const x = canEnterMazePosition(stepX, current.y, unlockedRef.current) ? stepX : current.x;
        const y = canEnterMazePosition(x, stepY, unlockedRef.current) ? stepY : current.y;
        const next = { x, y };
  ...
  return (
    <div className={`topdown-game ${Math.hypot(input.current.x,input.current.y)>.1?'topdown-moving':''} ${caught ? 'topdown-caught' : ''}`}>
      ...
      <div className={`portal-compass ${portalOpen ? 'portal-ready' : ''}`}>
        ...
        <span>{portalOpen ? 'ПОРТАЛ' : 'ДВЕРЬ ДЛЯ ВЗЛОМА'} · {Math.round(portalDistance)} м</span>
      </div>
      {portalDistance < 13 && !portalOpen && <div className="hack-prompt"><kbd>E</kbd> ВЗЛОМАТЬ ДВЕРЬ</div>}
      <div className="topdown-world" style={{ transform: `translate(${camera.x}%,${camera.y}%)` }}>
        ...
        {mazeWalls.map((wall, index) => <i key={index} className="top-wall" style={{ left:`${wall.x/2}%`,top:`${wall.y/2}%`,width:`${wall.width/2}%`,height:`${wall.height/2}%` }} />)}
        {mazeDoors.map((door, index) => (
          <div key={index}>
